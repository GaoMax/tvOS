...
public class SwiftSpinner: UIView {
    ...
    private func spinOuter() {
        ...
        let duration = Double(Float(arc4random()) /  Float(UInt32.max)) * 2.0 + 0.039
        let randomRotation = Double(Float(arc4random()) /  Float(UInt32.max)) * M_PI_4 + M_PI_4
        
        //outer circle
        UIView.animateWithDuration(duration, delay: 0.0, usingSpringWithDamping: 0, initialSpringVelocity: 0.0, options: [], animations: {
            ...
            self.outerCircleView.transform = CGAffineTransformMakeRotation(self.currentOuterRotation)
                ...
                let waitDuration = Double(Float(arc4random()) /  Float(UInt32.max)) + 0.07
    ...
    private func spinInner() {
        ...
        let duration = Double(Float(arc4random()) /  Float(UInt32.max)) * 2.0 + 0.023
        let randomRotation = Double(Float(arc4random()) /  Float(UInt32.max)) * M_PI_4 + M_PI_4
        //inner circle
        UIView.animateWithDuration(duration, delay: 0.0, usingSpringWithDamping: 0, initialSpringVelocity: 0, options: [], animations: {
            self.currentInnerRotation += CGFloat(randomRotation)
            self.innerCircleView.transform = CGAffineTransformMakeRotation(self.currentInnerRotation)
            }, completion: {_ in
                let waitDuration = Double(Float(arc4random()) /  Float(UInt32.max)) + 0.03
                self.delay(seconds: waitDuration, completion: {
                    if self.animating {
                        self.spinInner()
                    }
